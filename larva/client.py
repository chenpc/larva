...
def larva_call(host, port, token, module, func, *args, **kwargs):
    ...
    params = [args, kwargs]
    result = requests.post('https://%s:%d/api/%s/%s' % (host, port, module, func), headers=headers, json=params)
...
class LarvaProxy(object):
    def __init__(self, host="127.0.0.1", port=80, username=None, password=None, token=None, method=None):
        if token is None:
            r = requests.post('https://%s:%d/auth' % (host, port), auth=(username, password))
            self.token = r.text
        else:
            self.token = token
        self.method = method
